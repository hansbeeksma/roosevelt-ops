...
export async function createStatusPageIncident(
  ...
  const incidentData: StatusPageIncident = {
    ...
    body:
      incident.description || 'We are investigating the issue and will provide updates shortly.',
  ...
  try {
    const response = await fetch(`https://api.statuspage.io/v1/pages/${pageId}/incidents`, {
      method: 'POST',
      headers: {
...
export async function resolveStatusPageIncident(
  ...
  const message =
    resolutionMessage || 'This incident has been resolved. All systems are operational.'
...
export async function getStatusPageComponents(): Promise<StatusPageComponent[]> {
  ...
  try {
    const response = await fetch(`https://api.statuspage.io/v1/pages/${pageId}/components`, {
      headers: {
        Authorization: `OAuth ${apiKey}`,
      },
    })
