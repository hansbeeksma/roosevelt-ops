...
export function createValidationError(message: string): AppError {
  return new AppError(message, ErrorCodes.VALIDATION_ERROR, ErrorSeverity.LOW, 422)
}
...
export function createAuthError(message: string = 'Unauthorized'): AppError {
  return new AppError(message, ErrorCodes.UNAUTHORIZED, ErrorSeverity.MEDIUM, 401)
}
...
export function createForbiddenError(message: string = 'Access denied'): AppError {
  return new AppError(message, ErrorCodes.FORBIDDEN, ErrorSeverity.MEDIUM, 403)
}
...
export function createNotFoundError(message: string = 'Resource not found'): AppError {
  return new AppError(message, ErrorCodes.NOT_FOUND, ErrorSeverity.LOW, 404)
}
...
export function createRateLimitError(message: string = 'Too many requests'): AppError {
  return new AppError(message, ErrorCodes.RATE_LIMIT_EXCEEDED, ErrorSeverity.MEDIUM, 429)
}
...
export function createDatabaseError(message: string = 'Database operation failed'): AppError {
  return new AppError(message, ErrorCodes.DATABASE_ERROR, ErrorSeverity.CRITICAL, 500)
}
