...
describe('middleware', () => {
  ...
  describe('rate limiting', () => {
    ...
    it('returns 429 when rate limit exceeded', () => {
      ...
      const response = middleware(makeRequest('/api/example', { ip: '10.0.0.2' }))
    ...
    it('isolates rate limits per IP', () => {
      ...
      const response = middleware(makeRequest('/api/example', { ip: '10.0.0.4' }))
    ...
    it('applies stricter limits to slack routes', () => {
      ...
      const response = middleware(makeRequest('/api/slack/incident', { ip: '10.0.0.5' }))
      expect(response.status).toBe(429)
    })
  })
