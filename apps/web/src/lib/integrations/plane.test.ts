...
describe('createPlaneIncidentIssue', () => {
  ...
  it('creates incident issue with correct priority mapping', async () => {
    ...
    const result = await createPlaneIncidentIssue(makeIncident({ severity: 'SEV-2' }))
  ...
  it('includes slack channel URL in description when provided', async () => {
    ...
    await createPlaneIncidentIssue(makeIncident(), 'https://slack.com/channel/inc-001')
    ...
    const body = JSON.parse(createCall![1].body)
    expect(body.description_html).toContain('https://slack.com/channel/inc-001')
  })
...
describe('updatePlaneIncidentIssue', () => {
  ...
  it('returns false when not configured', async () => {
    vi.stubEnv('PLANE_API_KEY', '')
    vi.stubEnv('PLANE_WORKSPACE_SLUG', '')
    expect(await updatePlaneIncidentIssue('issue-1', makeIncident())).toBe(false)
  })
...
describe('addPlaneIssueComment', () => {
  ...
  it('adds comment to issue', async () => {
    ...
    const result = await addPlaneIssueComment('issue-1', 'Incident update: fix deployed')
    expect(result).toBe(true)
