 ...
 */
export async function acknowledgePagerDutyIncident(incidentId: string): Promise<boolean> {
  const routingKey = process.env.PAGERDUTY_ROUTING_KEY
 ...
 */
export async function resolvePagerDutyIncident(incidentId: string): Promise<boolean> {
  const routingKey = process.env.PAGERDUTY_ROUTING_KEY
 ...
 */
export async function getPagerDutyIncidentStatus(incidentId: string): Promise<{
  ...
  try {
    ...
    const response = await fetch(`https://api.pagerduty.com/incidents?incident_key=${incidentId}`, {
      headers: {
