...
describe('formatPagerDutyStatus', () => {
  ...
  it('formats acknowledged status', () => {
    expect(formatPagerDutyStatus('acknowledged')).toBe('🟡 PagerDuty: acknowledged')
  })
...
describe('triggerPagerDutyIncident', () => {
  ...
  it('returns null for SEV-3 incidents', async () => {
    const result = await triggerPagerDutyIncident(makeIncident({ severity: 'SEV-3' }))
    expect(result).toBeNull()
  })
  ...
  it('sends trigger event for SEV-1 incident', async () => {
    ...
    const result = await triggerPagerDutyIncident(makeIncident({ severity: 'SEV-1' }))
