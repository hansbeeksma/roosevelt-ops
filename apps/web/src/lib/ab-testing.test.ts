...
describe('getExperimentUserId', () => {
  ...
  it('creates and stores new user ID', () => {
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith('ab_test_user_id', userId)
  })

  it('returns existing user ID from localStorage', () => {
    ;(localStorage.getItem as ReturnType<typeof vi.fn>).mockReturnValue('user_existing_123')
    const userId = getExperimentUserId()
