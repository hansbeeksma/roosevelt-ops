...
describe('getStatusPageUrl', () => {
  ...
  it('returns statuspage.io URL with incident ID', () => {
    vi.stubEnv('STATUSPAGE_PAGE_ID', 'abc123')
    expect(getStatusPageUrl('inc-456')).toBe('https://abc123.statuspage.io/incidents/inc-456')
  })
...
describe('createStatusPageIncident', () => {
  ...
  it('creates incident with correct impact mapping', async () => {
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ id: 'sp-inc-1', shortlink: 'https://stspg.io/abc' }),
    })

    const result = await createStatusPageIncident(makeIncident({ severity: 'SEV-2' }))
  ...
  it('sets component status when affected components provided', async () => {
    ...
    await createStatusPageIncident(makeIncident({ severity: 'SEV-1' }), ['comp-1', 'comp-2'])
...
describe('updateStatusPageIncident', () => {
  ...
  it('returns false when not configured', async () => {
    vi.stubEnv('STATUSPAGE_API_KEY', '')
    vi.stubEnv('STATUSPAGE_PAGE_ID', '')
    expect(await updateStatusPageIncident('sp-1', 'update', 'identified')).toBe(false)
  })
  ...
  it('updates incident with message and status', async () => {
    ...
    const result = await updateStatusPageIncident('sp-1', 'Fix deployed', 'monitoring')
