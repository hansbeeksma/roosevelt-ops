...
3. **Run locally** to verify the check works:
...
Slack alerts are sent to the webhook configured in `SLACK_MONITORING_WEBHOOK_URL` on every **status transition** (e.g. `healthy` → `down`, `down` → `healthy`).
...
3. **L3 — Incident declaration**: If the service remains `down` for more than 15 minutes, open a formal incident via the incident management flow (`/incident declare` in Slack).
4. **L4 — Post-mortem**: For any SEV-1/SEV-2 incident, complete a blameless post-mortem within 48 hours.

### Silencing Alerts

To temporarily suppress alerts during planned maintenance, remove or unset `SLACK_MONITORING_WEBHOOK_URL` in the API environment and redeploy. Restore it when maintenance is complete.

---

## 3. Adjusting Budget Thresholds

Budget configurations are defined in:

```
lib/cost/budget-config.ts
```
...
1. Update `monthlyLimit` or `alertThreshold` for the relevant service in `lib/cost/budget-config.ts`.
2. Commit and deploy — the next 09:00 cron run uses the new values.

### To add a new service

1. Add a new entry to `DEFAULT_BUDGETS` in `lib/cost/budget-config.ts`.
2. Ensure cost data for that service is written to the `service_costs` table via `recordCost()` in `lib/cost/cost-tracker.ts` with a matching `service` string and `period: 'YYYY-MM'`.
3. Commit and deploy.

### Budget Slack Channel

Budget alerts are sent to `SLACK_BUDGET_WEBHOOK_URL`. If that variable is not set, they fall back to `SLACK_MONITORING_WEBHOOK_URL`. Configure both in the API environment for separate Slack routing.
...
The Inngest Cloud dashboard is at:
...
If a run fails (e.g. Supabase was briefly unreachable), you can replay it:
